...
### Модель доверия внутри туннеля (важно)

Доверие в mesh **плоское**: периметр защищён (gossip биндится на mesh-IP, вход
через Noise + cluster-secret), но *внутри* туннеля любая нода ≈ любая нода.
gossip-клиент мерджит peer-list от случайного соседа — то есть одна
скомпрометированная нода может пытаться отравлять control plane. Что с этим
сделано и что остаётся:

- ✅ **Угон mesh-IP заблокирован.** `MergePeers` отвергает peer'а, чей `node_ip`
  уже принадлежит другому pubkey, лежит вне mesh-CIDR или невалиден — нельзя
  переписать cryptokey-routing (`/32`) на чужой ключ и перехватить трафик соседа.
  Отказы логируются (`gossip peer rejected`).
- ✅ **Identity на bootstrap сшита с ключом.** Seed регистрирует только тот pubkey,
  которым клиент реально прошёл Noise-handshake (`hs.PeerStatic()` == заявленный),
  — фантомный чужой ключ вписать нельзя.
- ⚠️ **Endpoint существующего соседа — пока на доверии.** Злая нода может
  разослать через gossip валидный-по-формату, но ложный endpoint легитимного
  соседа → трафик к нему пойдёт «в никуда» (DoS, перебивается каждый раунд).
  Валидируется только формат, не подлинность. Правильный фикс — **self-attested
  gossip** (каждая `PeerInfo` подписана приватником своего pubkey, чужой endpoint
  подделать нельзя) — запланирован на v0.2.
- ⚠️ **Cluster-secret единый, без revoke.** Утечка секрета ≡ root в mesh: атакующий
  может заджойнить ноды и исчерпать `/24` (253 адреса). Ротация/revoke — v0.2.
